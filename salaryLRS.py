...
try:
    ...
    encoder = OneHotEncoder(sparse_output=False, handle_unknown='ignore')  # Initialize the encoder
except Exception as e:
    st.error(f'Error loading files: {e}')
    st.stop()
...
def main():
    ...
    if st.button('Predict'):
        try:
            ...
            numeric_features_scaled = scaler.fit_transform(numeric_features)  # Use loaded scaler
            numeric_features_scaled_df = pd.DataFrame(numeric_features_scaled, columns=numeric_columns)
